...
テストファイルの名前は `test_<sketch 名>.py` のように、**プロジェクト全体で一意になるよう**付けてください。同じ名前のファイルが 2 つあると、全体を流したときに collection のエラーになります。**既定の対象は、設定を書かなくても正しくなります。** pytest は最初から `.venv` のような隠しディレクトリや `build`、`dist` に降りていきません。そこへ**「既定で流したくないものは `test_` で始めない」**を足せば、それだけで揃います。

```bash
cd tests
uv run --env-file .env pytest    # unit、single、loopback、peer が対象
```

**設定を書かない、が最も安全です。** 対象を列挙する設定（`testpaths`）も、除外する設定（`norecursedirs`）も、書いた瞬間に**書き忘れや上書きの事故が入る余地**が生まれます。既定の実行を正しく保つのに、どちらも必要ありません。
