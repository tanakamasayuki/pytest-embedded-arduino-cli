...
一時的にテストを止めたいときにも使えますが、その用途では勧めません。名前を変えると、止めていること自体が見えなくなります。理由が残る方法を選んでください。
...
- **臨時の機材。** 特定のテストのときだけ繋ぐものです。追加の board、センサ、アナライザ、給電を切れる装置などです。

**分ける基準は台数ではなく、既定の実行でいつも使えるかどうかです。** 常設が 3 台あるなら 3 台のテストを既定に入れてよく、常設が 1 台なら 2 台のテストが臨時扱いになります。
...
時間が非常にかかるテストや、人の操作が必要なテストは、機材があっても毎回流したくないことがあります。方法は 2 つです。
...
`tests/` の下には、ボードを使わない純粋な Python のテストも置けます。その fixture は普通の pytest の使い方で、この plugin とは無関係です。
