...
- (EN) Expanded and reorganized `examples/` and related documentation, including basic usage samples, persistence / erase behavior, Unity-based testing, and an independent Arduino library project example.
- (JA) `examples/` と関連ドキュメントを拡充・整理し、基本的な使い方のサンプル、永続領域と erase 挙動、Unity ベースのテスト、独立した Arduino ライブラリプロジェクト例を追加・再構成。
